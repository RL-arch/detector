...
def difference(folder_start, folder_end, output_folder, extention):
    ...
    for file in os.listdir(folder_start):
        ...
        if file == (folder_start or folder_end
                    or output_folder) + "/" + '.DS_Store':
            continue
        filename = os.fsdecode(file)
        filename_2 = filename.replace("t02", "t12")

        # assign images
        img1 = Image.open(folder_start + filename)
        img2 = Image.open(folder_end + filename_2)
        ...
        # saving
        save_dir = f"{output_folder}/diff_images/" + filename.replace(
            extention, "_diff.png")
